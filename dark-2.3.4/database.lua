return {
	["melenchon"] = {
		name = "Mélenchon",
		firstname = "Jean-Luc",
		gender = "M",
	
		birth = "19 août 1951",
		death = nil,
		birthplace = "Tanger (Maroc)",
		
		bord = "gauche radicale",
	
		famille = {
			[1] = {
				name = "Georges Melenchon",
				link = "père",
				profession = "receveur des postes",
				birth = "12 novembre 1912",
				death = "26 janvier 1996",
			},
			[2] = {
				name = "Jeanine Bayona",
				link = "mère",
				profession = "institutrice",
				birth = "12 octobre 1912",
				death = "26 janvier 1996",
			},
			[3] = {
				name = "Bernadette Abriel",
				link = "ex-femme",
				profession = "",
				birth = "1 novembre 1952",
				death = "26 janvier 1996",
			},
			
		},
		profession = {
			[1] = {
				intitule = "Président du groupe La France Insoumise",
				date_deb = "21/02/2001",
				date_fin = nil,
				employeur = "L'état",
			},
			
		},
		parti = {
			[1] = "Parti Socialiste",
			[2] = "Parti de gauche",
			[3] = "Front de gauche",
			[4] = "France Insoumise",
			
		},
		formation = {
			[1] = {
				name = "Baccalaureat",
				date = "1969",
				lieu = "Lycée Rouget et Lisle",
				mention = nil
			},
			[2] = {
				name = "Licence de philosophie",
				date = "1972",
				lieu = "Faculté de lettres et sciences humaines, Besançon",
				mention = nil

			},
		}
	},
	["macron"] = {
		name = "Macron",
		firstname = "Emmanuel",
		gender = "M",
	
		birth = "21 Décembre 1977",
		death = nil,
		birthplace = "Amiens (France)",
		
		bord = "ni de gauche ni de doite",
	
		famille = {
			[1] = {
				name = "Jean-Michel Macron",
				link = "père",
				profession = "médecin, professeur de neurologie au CHU d'Amiens",
				birth = "1950",
				death = "26 janvier 1996",
			},
			[2] = {
				name = "Françoise Noguès",
				link = "mère",
				profession = "médecin et conseillère de la sécurité sociale",
				birth = "12 octobre 1912",
				death = "26 janvier 1996",
			},
		},
		profession = {
			[1] = {
				intitule = "Président de la République Française",
				date_deb = "14 mai 2017",
				date_fin = nil,
				employeur = "L'état",
			},
			[2] = {
				intitule = "Inspecteur des finances",
				date_deb = "2008",
				date_fin = "2010",
				employeur = "la banque d'affaires Rothschild & Cie",
			},	
		},
		parti = {
			[1] = "En Marche",
			[2] = "Parti Socialiste",
		},
		formation = {
			[1] = {
				name = "Baccalaureat, section scientifique",
				date = "1995",
				lieu = "Paris au lycée Henri-IV",
				mention = "très bien"
			},
			[2] = {
				name = "classes préparatoires de lettres",
				date = "1998",
				lieu = " Paris au lycée Henri-IV",
			},
			[3] = {
				name = "diplôme de Sciences Po",
				date = "2001",
				lieu = "l'Institut d'études politiques de Paris",
			},
		}
	},
}

